...
class LivroController {
  ...
  Future<Response> adicionar(Request req) async {
    ...
    final livro = Livro(
      ...
      status:
          StatusLeituraExtension.fromString(data['status']) ??
          StatusLeitura.naoLido,
      anotacoes: data['anotacoes'],
      avaliacao: data['avaliacao'] != null
          ? (data['avaliacao'] as num).toDouble()
          : null,
      emprestimo: data['emprestimo'] != null
          ? Emprestimo.fromJson(data['emprestimo'])
          : null,
    );

    await _repo.adicionar(livro);
    return Response.ok(
      jsonEncode({'message': 'Livro adicionado'}),
  ...
  Future<Response> atualizar(Request req, String idStr) async {
    ...
    return Response.ok(
      jsonEncode({'message': 'Livro atualizado'}),
      headers: {'Content-Type': 'application/json'},
    );
  }
  ...
  Future<Response> remover(Request req, String idStr) async {
    ...
    return Response.ok(
      jsonEncode({'message': 'Livro removido'}),
      headers: {'Content-Type': 'application/json'},
    );
  }
